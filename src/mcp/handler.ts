...
export const INSTRUCTIONS = [
  ...
  "Matching is on whole path segments, so /photos does not hold /photos-archive/lessons. One exception to the rule: / is not a bundle, since it would hold the entire site, so list_bundle and delete_bundle refuse it. A resource may still sit at /.",
  ...
  "Assets go in through upload_asset and come back out over ordinary HTTP. No tool hands back the contents of a file: a reply is text that lands in your context, and a photograph spent there is one nobody can look at. To read a stored file, fetch the url that list_assets and upload_asset give you and save it where you are working. A file under a private path answers that fetch with the same nothing everyone else gets, unless the request carries an API token, so a client holding no token sees a private asset as missing rather than closed.",
  ...
  "For a bike route, prefer chooses the trade: safety, balanced or speed, cycling only. It is not a label. On one 67 km ride, safety spent 1.6 km extra to cut main-road riding from 4.5 km to 2.7 km and put 93% of the route on signed cycle ways, while speed took 49 km of the same ride onto primary and secondary roads. Ask the owner which they want rather than assuming, and say what it cost.",
  "Then read ways in the reply before telling anyone a route is rideable: metres by surface, metres by highway type, how much is on a signed cycle route, and warnings for explicit prohibitions such as bicycle=no. Say what you found, including the bad stretches, and put them on the page. The stored asset carries a per-way segments table keyed to coordinate indices, so a page can colour the line by surface and point at the stretch it warns about.",
  "None of that is a safety score and you should not present it as one: safe depends on the rider, and this is only what OpenStreetMap records. analyzed_m says how much of the route carried tags at all and is 0 when the router reports none, which means nothing was examined rather than nothing being wrong. Untagged ground is counted under its own name for the same reason. Never report a clean route without saying how much of it was actually looked at.",
